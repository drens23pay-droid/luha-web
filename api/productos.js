// LUHA — Productos fijos (streaming/cursos). GET público (activos); POST admin.
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      const acc = b.action;
      if (acc === 'crear') {
        const r = await fetch(URL+'/rest/v1/productos', { method:'POST', headers:Object.assign({},H,{Prefer:'return=representation'}), body:JSON.stringify(b.data||{}) });
        return res.status(200).json({ ok:true, data:await r.json() });
      }
      if (acc === 'editar') {
        const r = await fetch(URL+'/rest/v1/productos?id=eq.'+encodeURIComponent(b.id), { method:'PATCH', headers:Object.assign({},H,{Prefer:'return=representation'}), body:JSON.stringify(b.data||{}) });
        return res.status(200).json({ ok:true, data:await r.json() });
      }
      if (acc === 'borrar') {
        await fetch(URL+'/rest/v1/productos?id=eq.'+encodeURIComponent(b.id), { method:'DELETE', headers:H });
