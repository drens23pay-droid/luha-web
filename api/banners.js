// LUHA — Banners del carrusel del home. GET público (activos, ordenados); POST admin (crear/editar/borrar).
module.exports = async (req, res) => {
  ...
  try {
    if (req.method === 'GET') {
      ...
      const r = await fetch(URL+'/rest/v1/banners'+q, { headers:H });
    ...
    if (req.method === 'POST') {
      ...
      const acc = b.action;
      if (acc === 'crear') {
        const r = await fetch(URL+'/rest/v1/banners', { method:'POST', headers:Object.assign({},H,{Prefer:'return=representation'}), body:JSON.stringify(b.data||{}) });
        return res.status(200).json({ ok:true, data:await r.json() });
      }
      if (acc === 'editar') {
        const r = await fetch(URL+'/rest/v1/banners?id=eq.'+encodeURIComponent(b.id), { method:'PATCH', headers:Object.assign({},H,{Prefer:'return=representation'}), body:JSON.stringify(b.data||{}) });
        return res.status(200).json({ ok:true, data:await r.json() });
      }
      if (acc === 'borrar') {
        await fetch(URL+'/rest/v1/banners?id=eq.'+encodeURIComponent(b.id), { method:'DELETE', headers:H });
